...
public class Signup extends Controller {
	...
	private static final Form<PasswordReset> PASSWORD_RESET_FORM = Form.form(PasswordReset.class);
	...
	public static Result doForgotPassword() {
		...
		if (filledForm.hasErrors()) {
			...
			final AppUser user = AppUser.findByEmail(email);
			if (user != null) {
	...
	public static Result doResetPassword() {
		...
		if (filledForm.hasErrors()) {
			...
			try {
				...
				flash(util.ConfigParameter.FLASH_MESSAGE_KEY, Messages.get("playauthenticate.reset_password.message.no_password_account"));
			}
			
			final boolean login = MyUsernamePasswordAuthProvider.getProvider().isLoginAfterPasswordReset();
			if (login) {
				// automatically log in
				flash(util.ConfigParameter.FLASH_MESSAGE_KEY, Messages.get("playauthenticate.reset_password.message.success.auto_login"));

				return PlayAuthenticate.loginAndRedirect(ctx(), new MyLoginUsernamePasswordAuthUser(u.email));
			} else {
				// send the user to the login page
				flash(util.ConfigParameter.FLASH_MESSAGE_KEY, Messages.get("playauthenticate.reset_password.message.success.manual_login"));
			}
			
			return redirect(controllers.account.routes.AccountMgmt.login());
		}
	}
	...
	public static Result verify(final String token) {
		...
		AppUser.verify(ta.targetUser);
		flash(util.ConfigParameter.FLASH_MESSAGE_KEY,
				Messages.get("playauthenticate.verify_email.success", email));
		if (controllers.account.AccountMgmt.getLocalUser(session()) != null) {
			return redirect(controllers.routes.Application.index());
		} else {
			return redirect(controllers.account.routes.AccountMgmt.login());
		}
