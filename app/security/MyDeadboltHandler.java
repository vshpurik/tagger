...
public class MyDeadboltHandler extends AbstractDeadboltHandler
{
	...
	public Result beforeAuthCheck(final Http.Context context) {
		if (PlayAuthenticate.isLoggedIn(context.session())) {
			...
			context.flash().put(util.ConfigParameter.FLASH_ERROR_KEY, "Please login to access this feature");
	...
	public Subject getSubject(final Http.Context context) {
		...
		return AppUser.findByAuthUserIdentity(u);
	}
	...
	public Result onAuthFailure(Http.Context context, String content) {
		AppUser appUser = (AppUser) context.args.get("deadbolt.cache-user");

		String msg;
		if (appUser == null)
			msg = getMessage(context);
		else
			msg = "Access denied (appUserId=" + appUser.appUserId + ").";
		
    	Logger.info(msg);

		final String originalUrl = PlayAuthenticate.storeOriginalUrl(context);
		context.flash().put(util.ConfigParameter.FLASH_ERROR_KEY, msg);
		return redirect(PlayAuthenticate.getResolver().login());
	...
	private String getMessage(Http.Context context) {
